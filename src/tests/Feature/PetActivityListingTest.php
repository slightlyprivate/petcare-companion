...
class PetActivityListingTest extends TestCase
{
    ...
    public function test_lists_paginated_activities_with_default_per_page(): void
    {
        ...
        $response = $this->actingAs($user, 'sanctum')->getJson('/api/pets/'.$pet->getKey().'/activities');
    ...
    public function test_filters_by_type(): void
    {
        ...
        $response = $this->actingAs($user, 'sanctum')->getJson('/api/pets/'.$pet->getKey().'/activities?type=feeding');
        $response->assertStatus(200)
    ...
    public function test_filters_by_date_range(): void
    {
        ...
        $response = $this->actingAs($user, 'sanctum')->getJson('/api/pets/'.$pet->getKey().'/activities?date_from='.$from.'&date_to='.$to);
        $response->assertStatus(200)
            ->assertJsonPath('meta.total', 2)
            ->assertJsonCount(2, 'data');
    }
