...
- Start PWA dev server: `npm install && npm run dev` inside `src/pwa`
...
- `lib/`: Cross-cutting utilities used by more than one domain (e.g., HTTP clients, CSRF helpers,
  query wrappers, notifications, config, brand utilities). These should be UI-agonistic and
  reusable.
...
- Centralized route paths live in `src/pwa/src/routes/paths.ts` as `PATHS` constants.
- Always import and use `PATHS` instead of hardcoding strings like `/dashboard` or `/care`.
- Route map (PWA):
  - Public: `/` (Landing), `/caregiver-invitations/accept`
  - Auth: `/auth/signin`, `/auth/signup`, `/auth/verify`
  - Pet-focused: `/dashboard` (index → My Pets), `/dashboard/pets`, `/dashboard/pets/new`,
    `/dashboard/pets/:id`, `/dashboard/pets/:id/settings`
  - Caregiving flows: `/dashboard/activities`, `/dashboard/routines`
  - Not Found: `*`

### Linting and route guard

- A lightweight guard prevents introducing hardcoded PWA route strings:
...
- ESLint config (`.eslintrc.cjs`) includes `no-restricted-syntax` selectors that will flag common
  patterns. Run `npm run lint` after adding ESLint to your dev deps.
...
- See `.env.example` in `src/pwa` for typical values.

### Container runtime (production/staging)

- Set `API_BASE_URL` on the PWA container to point at the Laravel Nginx service (e.g.,
  `http://petcare_web_staging`). The Experience UI calls `/api/...` and Nginx proxies at runtime via
  a templated config.

## CSRF protection

- The PWA uses Laravel Sanctum's standard CSRF flow via `GET /sanctum/csrf-cookie`, which sets the
  `XSRF-TOKEN` cookie.
...
- Import domain clients and hooks via `src/pwa/src/api/index.ts` to simplify imports as features
  grow: `import { pets, petHooks } from '@/api'`.

## Query helpers

- Prefer `useAppQuery` / `useAppMutation` from `src/pwa/src/lib/appQuery` for consistent defaults.
...
- For cursor/page flows, use `usePaginatedQuery` to keep previous data while fetching the next page.
  Example:
  `usePaginatedQuery({ queryKey: [qk.pets.all, page], queryFn: () => listPetActivities({ page }), keepPreviousData: true })`.

## Cache invalidation patterns

- `src/pwa/src/lib/queryUtils.ts` exposes helpers to reduce manual cache touches:
...
- Favor primitives under `src/pwa/src/components/` to keep Tailwind consistent:
...
- CSRF behavior: Manually validate `GET /sanctum/csrf-cookie` sets the XSRF-TOKEN cookie. The PWA
...
- `src/pwa/src/lib/optimistic.ts` provides `optimisticListUpdate` to stage UI updates before the
